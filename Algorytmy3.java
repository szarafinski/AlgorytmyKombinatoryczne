...
public class Algorytmy3 {
    ...
    void wylicz(int n) {
        ...
        while (czyDalej(dane, kierunek)) {
            int aktualnaMobilna = znajdzMaxMobilna(dane, kierunek);
    ...
    int[] stworzTablice(int n) {
        int[] tablica = new int[n];
        for (int i = 0; i < n; i++) {
            tablica[i] = Kierunek.LEWO.wartosc;
        }
        return tablica;
    }
    ...
    int znajdzMaxMobilna(int[] tablicaRobocza, int[] tablicaKierunkowa) {
        ...
        for (int i = 0; i < tablicaRobocza.length; i++) {
         ...
         System.out.println("minimalna mobilna to: " + najwieksza + " na pozycji " + pozycja);
        return pozycja;
    }
